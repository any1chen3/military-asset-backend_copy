...
import com.military.asset.utils.*;
...
public class DataContentAssetServiceImpl extends ServiceImpl<DataContentAssetMapper, DataContentAsset> implements DataContentAssetService {
     ...
     *
     * 🎯 场景E：跨表同步触发
     *   - 单位在网信表存在记录 → 同步省市信息到网信表
     *   - 单位在网信表不存在记录 → 跳过跨表同步
     *
     * ==================== 同步机制说明 ====================
     * 1. 本表级联更新：确保同一单位在data_content_asset表中省市一致
     * 2. 上报单位表同步：更新单位表省份信息和dataContent状态位
     * 3. 跨表同步：确保相同单位在cyber_asset表中省市一致
     *
     * 数据校验规则（按字段顺序）：
    ...
    public void add(DataContentAsset asset) {
        ...
        // 🎯 场景E：跨表同步处理
        // 如果单位在网信表存在记录，同步省市信息到网信表
        handleCrossSyncForAdd(asset);

        log.info("🎉 数据内容资产新增完整流程完成，ID：{}", asset.getId());
    }
...
// ==================== 新增的智能省市处理方法 ====================
     ...
     * 🎯 场景A：用户输入了有效省市（最高优先级）
     *   - 条件：用户输入的省市都有效且逻辑一致
     *   - 处理：直接使用用户输入，只进行标准化处理
     *   - 示例：用户输入"江苏省-南京市" → 使用"江苏省-南京市"
     *
     * 🎯 场景B：上报单位表存在有效省份
     *   - 条件：单位表中该单位存在且省份字段有效
     *   - 处理：使用单位表省份，根据省份推导城市（使用省份首府）
     *   - 示例：单位表有"浙江省" → 使用"浙江省-杭州市"
     *   - 注意：上报单位表只存储省份，不存储城市
     *
     * 🎯 场景C：智能推导完整省市
     *   - 条件：用户输入无效且单位表不存在或省份无效
     *   - 处理：基于单位名称进行完整省市推导
     *   - 示例：单位"某部队" → 推导为"江苏省-南京市"
     *   - 兜底：推导失败时设为"未知-未知"
     *
     * @param asset 新增资产对象
     */
    private void handleProvinceCityForAdd(DataContentAsset asset) {
        String userProvince = asset.getProvince();
        String userCity = asset.getCity();
        String reportUnit = asset.getReportUnit();

        log.info("🤖 新增智能省市处理 - 单位: {}, 用户输入: {}-{}",
        ...
        log.info("✅ 新增省市处理完成 - 最终: {}-{}", asset.getProvince(), asset.getCity());
    }
     ...
     */
    private void useReportUnitProvince(DataContentAsset asset, ReportUnit reportUnit) {
...
// ==================== 新增的级联更新方法 ====================
     ...
     *
     * 🎯 触发条件：
     *   - 该单位在 data_content_asset 表中已存在其他记录
     *   - 新增记录的最终省市与现有记录不一致
     *
     * 🎯 不触发条件：
     *   - 单位在本表无其他记录（首次使用该单位）
     ...
     * @param unitName 单位名称
     */
    private void handleCascadeUpdateForAdd(DataContentAsset asset, String unitName) {
        ...
        try {
            // 查询该单位在本表的所有记录
            List<DataContentAsset> sameUnitAssets = baseMapper.selectByReportUnitExcludeId(unitName, asset.getId());
            ...
            for (DataContentAsset existingAsset : sameUnitAssets) {
                // 检查是否需要更新
                ...
                if (needsUpdate) {
                    ...
                    existingAsset.setCreateTime(LocalDateTime.now());
...
// ==================== 新增的跨表同步方法 ====================
     ...
     * @param asset 新增资产对象
     */
    private void handleCrossSyncForAdd(DataContentAsset asset) {
        ...
        try {
            // 🎯 关键条件：检查单位在网信资产表中是否存在
            boolean unitExistsInCyberTable = checkUnitExistsInCyberTable(unitName);
            if (!unitExistsInCyberTable) {
                log.debug("⏭️ 场景E跳过：单位[{}]在网信资产表中不存在", unitName);
                return;
            }

            log.info("🎯 场景E触发：单位[{}]在网信表存在，开始跨表同步 → {}-{}", unitName, province, city);
            syncToCyberTable(unitName, province, city);
     ...
     */
    private boolean checkUnitExistsInCyberTable(String unitName) {
        try {
            // 使用网信资产Mapper查询该单位是否存在记录
            Long count = cyberAssetMapper.countByReportUnit(unitName);
            boolean exists = count != null && count > 0;
            log.debug("🔍 单位存在性检查：[{}]在网信资产表中{}存在", unitName, exists ? "" : "不");
            return exists;
        } catch (Exception e) {
...
// ==================== 金额计算方法 ====================
     ...
     *
     * ==================== 与网信资产的区别 ====================
     * 1. 校验规则不同：数据资产有特有的应用领域、开发工具、更新周期、更新方式等字段校验
     * 2. 跨表同步方向：数据表 → 网信表（与网信表相反）
     * 3. 业务逻辑相同：省市推导、状态同步等核心逻辑与网信资产保持一致
     *
     * ==================== 同步触发优化 ====================
     * 🆕 阶段7（上报单位表同步）：单位变更 OR 用户修改省市 OR 系统自动修正省市
     * 🆕 阶段8（跨表同步）：单位存在 AND 省市实际发生变化
     *
     * ==================== 事务管理 ====================
    ...
    public void update(DataContentAsset asset) {
        ...
        log.info("✅ [阶段5] 修改数据内容资产成功，ID：{}，资产名称：{}", asset.getId(), asset.getAssetName());

        // ==================== 阶段6：上报单位表同步 ====================
        log.debug("🔄 [阶段6] 开始上报单位表同步");
     ...
     * @param existingAsset 原始资产对象（用于获取原始信息）
     */
    private void handleClearedProvinceCity(DataContentAsset asset, DataContentAsset existingAsset) {
        String userProvince = asset.getProvince();
        String userCity = asset.getCity();
        String originalProvince = existingAsset.getProvince();
        String originalCity = existingAsset.getCity();
     ...
     * @param asset 当前资产对象
     */
    private void handleProvinceCityValidity(DataContentAsset asset) {
        String userProvince = asset.getProvince();
        String userCity = asset.getCity();
    ...
    private void handleCascadeUpdateForUnit(String unitName, String newProvince, String newCity, String excludeId) {
        ...
        try {
            ...
            List<DataContentAsset> sameUnitAssets = baseMapper.selectByReportUnitExcludeId(unitName, excludeId);
            ...
            // 2. 批量更新这些记录
            int updatedCount = 0;
            for (DataContentAsset asset : sameUnitAssets) {
                // 记录原始值用于日志
    ...
    private boolean needCrossTableSync(String newUnit, String oldProvince, String oldCity,
        ...
        // 🎯 条件3：检查单位在网信资产表中是否存在
        boolean unitExists = checkUnitExistsInCyberTable(newUnit);
        if (!unitExists) {
            log.debug("⏭️ 跨表同步跳过：单位在网信资产表中不存在 - {}", newUnit);
            return false;
        }
...
//    }
    ...
    @Override
    public DataAssetReportUnitAnalysisVO analyzeReportUnitDomainAndCycle(String reportUnit) {
        ...
        List<Map<String, Object>> fieldRows = dataContentAssetMapper.countApplicationFieldByReportUnit(reportUnit);
        List<Map<String, Object>> cycleRows = dataContentAssetMapper.countUpdateCycleByReportUnit(reportUnit);

        Map<String, Long> rawFieldCounts = fieldRows.stream()
                .collect(Collectors.toMap(
                        row -> (String) row.get("applicationField"),
                        row -> ((Number) row.get("count")).longValue(),
                        Long::sum,
                        LinkedHashMap::new
                ));

        Map<String, Long> rawCycleCounts = cycleRows.stream()
                .collect(Collectors.toMap(
                        row -> (String) row.get("updateCycle"),
                        row -> ((Number) row.get("count")).longValue(),
                        Long::sum,
                        LinkedHashMap::new
                ));

        Map<String, Long> applicationFieldCounts = DataAssetAnalysisFormulaUtils.normalizeCountMap(
                DataAssetAnalysisFormulaUtils.STANDARD_APPLICATION_FIELDS, rawFieldCounts);
        Map<String, Long> updateCycleCounts = DataAssetAnalysisFormulaUtils.normalizeCountMap(
                DataAssetAnalysisFormulaUtils.STANDARD_UPDATE_CYCLES, rawCycleCounts);

        Map<String, BigDecimal> applicationFieldRatios = DataAssetAnalysisFormulaUtils.calculateRatios(applicationFieldCounts);
        Map<String, Long> categoryCounts = DataAssetAnalysisFormulaUtils.aggregateByFunctionCategory(applicationFieldCounts);
        Map<String, BigDecimal> categoryRatios = DataAssetAnalysisFormulaUtils.calculateRatios(categoryCounts);
        Map<String, BigDecimal> updateCycleRatios = DataAssetAnalysisFormulaUtils.calculateRatios(updateCycleCounts);

        String functionCategory = DataAssetAnalysisFormulaUtils.determineFunctionCategory(categoryRatios);
        List<String> imbalanceWarnings = DataAssetAnalysisFormulaUtils.detectImbalance(applicationFieldRatios, categoryRatios);

        BigDecimal dependencyScore = DataAssetAnalysisFormulaUtils.calculateDependencyScore(updateCycleCounts);
        String dependencyLevel = DataAssetAnalysisFormulaUtils.determineDependencyLevel(dependencyScore);
        String updateRhythm = DataAssetAnalysisFormulaUtils.determineUpdateRhythm(updateCycleCounts);

        DataAssetReportUnitAnalysisVO vo = new DataAssetReportUnitAnalysisVO();
        vo.setReportUnit(reportUnit);
        vo.setApplicationFieldCounts(applicationFieldCounts);
        vo.setApplicationFieldRatios(applicationFieldRatios);
        vo.setDomainCategoryCounts(categoryCounts);
        vo.setDomainCategoryRatios(categoryRatios);
        vo.setUpdateCycleCounts(updateCycleCounts);
        vo.setUpdateCycleRatios(updateCycleRatios);
        vo.setFunctionCategory(functionCategory);
        vo.setImbalanceWarnings(imbalanceWarnings);
        vo.setDependencyScore(dependencyScore);
...
// ============================ 🆕 新增方法（清空再导入专用） ============================
    ...
    /**
     * 批量保存数据内容资产并同步省市信息（导入专用-完整16种情况处理版）
     *
     * ==================== 核心特性 ====================
     * ✅ 完整的16种输入情况覆盖
     * ✅ 基于状态判断的精确处理逻辑
        ...
        try {
            ...
            for (Map.Entry<String, List<DataContentAsset>> entry : unitGroupedAssets.entrySet()) {
                ...
                syncRequests.add(new ProvinceAutoFillTool.UnitSyncRequest(
                        unitName,
                        firstAsset.getProvince(),  // 使用数据内容资产的省市信息
                        "dataContent",             // 资产类型标识
                        false                      // 新增模式
                ));
            }
            ...
            for (Map.Entry<String, List<DataContentAsset>> entry : unitGroupedAssets.entrySet()) {
                ...
                // 🎯 无论省市是什么，只要网信表存在就同步
                if (checkUnitExistsInCyberTable(unitName)) {
                    syncToCyberTable(unitName, province, city);
                    crossSyncCount++;
            ...
            log.info("🎉 数据内容资产批量导入完成，涉及{}个单位，跨表同步{}个单位", unitGroupedAssets.size(), crossSyncCount);
     ...
     * 4. 结果标准化（确保输出格式统一）
     */
    private void processProvinceCityForBatchImport(List<DataContentAsset> assets) {
        ...
        log.info("📊 数据内容资产批量省市处理统计 - 场景1: {}, 场景2: {}, 场景3: {}, 场景4: {}, 场景5: {}",
     ...
     * 🎯 场景1：Excel省市都有效且一致 → 使用Excel值（标准化）
     */
    private void handleScene1(DataContentAsset asset, String province, String city) {
     ...
     * 🎯 场景2：Excel省市都为空 → 单位推导
     */
    private void handleScene2(DataContentAsset asset, String unitName) {
     ...
     * 🎯 场景4：Excel省市都未知或都无效 → 省市都设为未知
     */
    private void handleScene4(DataContentAsset asset) {
     ...
     * 🎯 场景5：Excel省市不一致 → 市设为未知，省保留
     */
    private void handleScene5(DataContentAsset asset, String province) {
     ...
     */
    private void handleScene3(DataContentAsset asset, FieldState provinceState, FieldState cityState,
     ...
     * 🎯 场景3的其他情况处理
     */
    private void handleOtherScene3Cases(DataContentAsset asset, FieldState provinceState, FieldState cityState,
                                        String processedProvince, String processedCity, String unitName) {
     ...
     * 🎯 仅推导省份（保持城市不变）
     */
    private void useToolToDeriveProvince(DataContentAsset asset, String unitName) {
        String originalCity = asset.getCity();
     ...
     */
    private void applyFinalStandardization(DataContentAsset asset) {
        // 只有非"未知"的值才需要标准化
