...
//     * 3.11 盘点单位校验：必填
     ...
     *
     * @apiNote 本方法是系统中业务逻辑最复杂的方法之一，修改时需谨慎测试所有场景
     */
    @Override
    @Transactional(rollbackFor = Exception.class)
...
//        validateProvinceCity(asset.getProvince(), asset.getCity());
        ...
        log.info("✅ [阶段6] 修改网信资产成功，ID：{}，资产名称：{}", asset.getId(), asset.getAssetName());
     ...
     * 新增：使用标准化的省市名称处理逻辑，确保数据格式统一。

     * ==================== 处理逻辑 ====================
     * 1. 直接使用用户输入的省市信息，不进行任何推导
     * 2. 使用标准化处理，确保数据格式统一（如"四川"→"四川省"）
     ...
     * ==================== 技术实现 ====================
     * 通过精确的变更检测，确定用户实际修改了哪些字段
     * 结合AreaCacheTool提供的省市映射关系，智能补全缺失信息
     *
     * @param asset 当前资产对象（包含用户修改后的数据）
        ...
        if (provinceChanged && cityChanged) {
            ...
            try {
                ...
                if (StringUtils.hasText(capital)) {
                    ...
                    log.warn("⚠️ 无法找到省份的首府，请检查修改的省份，便于恢复原城市信息");
     ...
     * 新增：使用标准化的简写匹配逻辑，提高单位名称匹配的准确性。

     * ==================== 优化策略 ====================
     * 策略1：查询上报单位表，如果单位存在且省份有效 → 直接使用该省份，补全首府
     * 策略2：如果单位不存在或省份无效 → 使用工具类智能推导（包含标准化简写匹配）
     *
     * @param asset 当前资产对象
     ...
     * ==================== 方法说明 ====================
     * 根据不同的变更情况，精确控制上报单位表的同步逻辑。
     * 支持三种主要场景：只修改单位、只修改省市、同时修改单位和省市。
     ...
     * ==================== 技术实现 ====================
     * 通过精确的条件判断，确定具体的同步场景
     * 调用ProvinceAutoFillTool的syncReportUnit方法进行实际同步
     * 记录详细的同步日志，便于问题排查
     *
     ...
     *
     * @apiNote 此方法确保在各种修改场景下，上报单位表的状态都能正确同步
     */
     ...
     * @return 是否需要跨表同步
     *
     * @apiNote 严格的触发条件避免不必要的跨表操作，提升系统性能
     */
