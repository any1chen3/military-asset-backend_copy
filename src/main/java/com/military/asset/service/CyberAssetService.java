...
/**
 * 网信资产业务层接口
 * 继承MyBatis-Plus的IService获取基础CRUD能力，新增网信特有功能（数量校验、数量范围查询）
 * 核心约束：已用数量（usedQuantity）≤ 实有数量（actualQuantity）
 ...
 * 注意：IDE可能显示"始终反转对布尔方法的调用"警告，这是业务逻辑需要，故意反转调用
 */
@SuppressWarnings("unused")
public interface CyberAssetService extends IService<CyberAsset> {

    /**
     * 批量保存网信资产 - 适配AssetImportController
     * 特性：事务保障（全成功/全失败），避免部分数据入库
     * @param validDataList 已转换为业务VO的合法Excel数据
     */
    void batchSaveCyberAssets(List<CyberAssetExcelVO> validDataList);

    /**
     * 查询所有已存在的网信资产ID（原有方法）
     * 用途：Excel导入时去重校验，避免重复入库
     * @return 数字+字母组合的ID列表
     */
    List<String> getExistingIds();

    /**
     * 批量保存Excel校验通过的网信资产数据（原有方法）
     * 特性：事务保障（全成功/全失败），避免部分数据入库
     * @param validVoList 已转换为业务VO的合法Excel数据
     */
    void batchSaveValidData(List<CyberAssetExcelVO> validVoList);

    /**
     * 处理Excel导入结果并记录审计日志
     * 用途：统计导入总条数、成功条数、失败条数，输出错误详情（行号/字段/原因）
     * @param totalRow 导入Excel的总行数
     * @param validRow 成功入库的合法条数
     * @param errorList 校验失败的错误详情列表
     */
    void handleImportResult(int totalRow, int validRow, List<ExcelErrorVO> errorList);

    /**
     * 按ID查询网信资产详情
     * 附加校验：ID格式（数字+字母组合）+ 数据库存在性
     * @param id 网信资产ID
     * @return 完整的网信资产实体
     * @throws RuntimeException ID格式错误或不存在时抛出
     */
    CyberAsset getById(String id);
    ...
    Object combinedQuery(Integer pageNum, Integer pageSize,
                         ...
                         String startUseDateStart, String startUseDateEnd,
                         String inventoryUnit);
    /**
     * 新增网信资产
     * 核心流程：自动填充省市 → 数据校验 → 保存资产 → 同步上报单位表
     * 同步逻辑：使用填充后的省市信息同步上报单位表，设置网信资产状态标志为1
     *
     * @param asset 网信资产对象
     */
    void add(CyberAsset asset);

    /**
     * 修改网信基础资产
     * 功能说明：
     * 根据资产ID修改网信资产记录，包含完整的数据校验、省市自动填充和上报单位表同步。
     ...
     * - 事务性操作确保数据一致性
     *
     * @param asset 网信资产对象（包含修改后的数据和原ID）
     * @throws RuntimeException 当资产不存在、数据校验失败或更新失败时抛出业务异常
     */
    void update(CyberAsset asset);

    /**
     * 删除网信基础资产
     * 功能说明：
     * 根据资产ID删除网信资产记录，并同步更新上报单位表的状态标志。
    ...
    /**
     * 校验分类编码与资产分类名称是否匹配
     * 匹配规则：从网信分类映射表获取标准名称，与传入名称比对（忽略前后空格）
     ...
     * @param actualQuantity 实有数量
     * @throws RuntimeException 数量为负或已用>实时时抛出
     */
    void validateUsedQuantity(Integer usedQuantity, Integer actualQuantity);
     ...
     * - 使用Map结构提供O(1)的查询性能
     *
     * @return Map<String, CyberAsset> 资产ID到完整资产对象的映射
     * @throws RuntimeException 当数据加载失败时抛出
     */
    Map<String, CyberAsset> getExistingAssetsMap();
     ...
     * @param asset 待新增的网信资产实体
     * @throws RuntimeException 校验失败或保存失败时抛出
     */
    void addCyberAsset(CyberAsset asset);

    /**
     * 修改网信资产（集成省市自动填充和上报单位同步）
     ...
     * - 需要重新推导省市的修改操作
     *
     * @param asset 含修改信息的网信资产实体
     * @throws RuntimeException 校验失败或更新失败时抛出
     */
    void updateCyberAsset(CyberAsset asset);
     ...
     */
    void deleteCyberAsset(String id);
     ...
     * @return 分页结果
     */
    Page<CyberAsset> queryByCategory(Page<CyberAsset> page, String categoryCode, String assetCategory);
     ...
     * @return 分页结果
     */
    Page<CyberAsset> queryByReportUnit(Page<CyberAsset> page, String reportUnit);

// ==================== 新增：接口4相关方法 ====================

    /**
     * 接口4(a)：获取网信资产表各省份单位数量统计（新逻辑：关联report_unit表）
     * 作用：统计网信资产表中每个省份包含的不同上报单位数量
     * 新逻辑说明：
     ...
     * 示例返回：[{"province": "北京", "count": 4}, {"province": "江苏", "count": 2}]
     * 注意：统一使用report_unit表的省份信息，确保三个资产表数据一致性
     */
     ...
     */
    void clearCyberTableAndResetStatus();
     ...
     */
    void batchSaveForImport(List<CyberAsset> assets);

    /**
     * 网信资产联合查询方法
     * 作用：根据动态条件查询网信资产数据，支持分页
     * 参数说明：所有查询条件参数都是可选的，根据前端实际传递的条件进行查询
     * 如果所有参数都为null，则返回全部数据
     */
    Page<CyberAsset> combinedQuery(Page<CyberAsset> pageInfo,
                                   String reportUnit, String province, String city,
